...
class ReadNetinitPhaseProcess(AbstractMultiConnectionProcess):
    ...
    def _process_p2p_guess(self, response):
        p2p_guess_y, p2p_guess_x = struct.unpack_from(
            "<hh", response.data, response.offset)
        if (p2p_guess_x != self._last_p2p_guess_x or
                p2p_guess_y != self._last_p2p_guess_y):
            self._last_p2p_guess_x = p2p_guess_x
            self._last_p2p_guess_y = p2p_guess_y
            warn("{:03d} {:03d} {:05d} thinks it is {} {}".format(
                self._x, self._y, self._index, p2p_guess_x, p2p_guess_y))
            self._index += 1
    ...
    def _process_p2p_dims_response(self, response):
        guess_y, guess_x, max_height, max_width = struct.unpack_from(
            "<BBBB", response.data, response.offset)
        if guess_x != self._last_x or guess_y != self._last_y:
            warn("{:03d} {:03d} {:05d} thinks it is {} {}".format(
                self._x, self._y, self._index, guess_x, guess_y))
            self._last_x = guess_x
            self._last_y = guess_y
            self._index += 1
        if (max_width != self._last_max_width or
                max_height != self._last_max_height):
            warn("{:03d} {:03d} {:05d} thinks machine is {} x {}".format(
                self._x, self._y, self._index, max_width, max_height))
...
class ReadBoardProcess(AbstractMultiConnectionProcess):
    ...
    def _read_board(self):
        ...
        try:
            ...
            warn("Warning:", x, y, "missing when asked from",
                 self._eth_x, self._eth_y, e)
...
class MainThread(object):
    def __init__(self, core_counter, job):
        self._done = False
        self._thread = Thread(target=self.run, args=[core_counter, job])
        ...
        with job:
            ...
            if boot_done:
                ...
                p2p_process = GetP2PTableProcess(
                    RoundRobinConnectionSelector(
                        [root_connection]), job.width, job.height)
