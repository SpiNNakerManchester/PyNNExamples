...
tp1 = np.array([250]).reshape(1, -1)
tp2 = np.array([1]*35)
tp3 = np.array([15]*1)

freq1 = np.arange(6, 30, 4.8)
freq2 = np.arange(30, 6, -0.8)
base0 = np.array([6]).reshape(1, -1)

tp100 = np.concatenate((tp2, tp3, tp2, tp3), axis=0)
tp500 = np.tile(tp100, 5).reshape(360, 1)

total_time = np.concatenate((tp1, tp500, tp1)).flatten()

freqchain1 = np.concatenate((freq1, freq2), axis=0).reshape(35, -1)
freqchain2 = freqchain1[::-1].reshape(35, -1)

oscillation_freq = np.concatenate((freqchain1, base0, freqchain2, base0), axis=0).flatten()
oscil_ten = np.tile(oscillation_freq, 5 ).reshape(360, -1)
total_freq = np.concatenate((base0, oscil_ten, base0)).round(decimals=1).flatten()

timefreq = np.stack((total_time, total_freq))
...
for i in range(362):
    ...
    run_for = (timefreq[0, i].astype(int))
    rate=timefreq[1, i]
    src_Thal.set(rate=rate)
    src_Thal.set(duration= run_for)
    print "Simulation running for {} milliseconds".format(run_for)
    p.run(run_for)
