...
target_pop_size = 64
num_pattern_neurons = 125#20#100#stim_size*0.1#100#
max_sync_pattern_neurons = num_pattern_neurons/2.
p_connect = 0.3#0.35#0.6#1.0#
...
num_patterns =2
noise_rate = 20.#TODO:derive equation for the ratio of this and num_pattern_neurons (SNR) from fan in
w2s =2.
w2s_target = 5.
w_min = 0.
if num_pattern_neurons>0:
    av_weight = w2s_target/(num_pattern_neurons)
    w_max = w2s_target / max_sync_pattern_neurons
else:
...
tau_plus=16
tau_minus=30.
a_plus =0.005#0.01#0.001#
a_minus =0.005#0.01#0.001#
nearest_pair = True
min_delay = 1.
max_delay = 51
...
if target_pop_size <= 100:
    ...
    spike_raster_plot_8(target_data.segments[0].spiketrains,plt,duration/1000.,target_pop_size+1,0.001,
                        title="target pop activity",filepath=results_directory,onset_times=onset_times,pattern_duration=300)
    spike_raster_plot_8(target_data.segments[0].spiketrains,plt,duration/1000.,target_pop_size+1,0.001,
                        title="target pop final activity",filepath=results_directory,xlim=(0.001*(final_pattern_start_time-1),0.001*duration),
                        onset_times=onset_times,pattern_duration=300)

weight_dist_plot(varying_weights,1,plt,0.0,w_max,title="pre-pop weight distribution",filepath=results_directory)

# selective_neuron_search(onset_times,target_data.segments[0].spiketrains,time_window=100.,
