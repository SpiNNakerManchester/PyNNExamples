...
def bar_plot(ax, data, colors=None, total_width=0.8, single_width=1, legend=True, perc=0, x_axis=[]):
    ...
    ax.set_title("Processed Synaptic Events Per Neuron " + str(perc) + " % connectivity (Plastic) 1 ms")
...
if __name__ == "__main__":
    ...
    for k in tot_events_multi.keys():
        ...
        fig, ax = plt.subplots()
        bar_plot(ax, data, total_width=.8, single_width=.9, perc=k, x_axis=x_axis, colors=plt.cm.viridis(np.linspace(0, 1, 3)))
        plt.show()
