...
import numpy as np
import pylab as plt
from signal_prep import spike_raster_plot_8,get_sub_pop_spikes
...
input_size = 10000.#len(input_spikes)#
n_total = 2 * input_size
target_pop_size = input_size#100.#int(np.round(n_total*10./89.)) #200
...
for _ in range(int(input_size)):
    input_spikes.append([isi_ms*i + (np.random.rand())*(isi_ms) for i in range(n_repeats)])
...
w2s_target = .1#7.#
av_weight =(w2s_target/25.)#w2s_target#
...
#keep record of original post neuron ids to use when organising output spikes
...
# source_target_proj = sim.Projection(source_pop,target_pop,sim.FromListConnector(source_target_list),synapse_type=sim.StaticSynapse(weight=av_weight))

# for i,pop in enumerate(input_pops):
#     input_index = i * pop.size
#     connection_list = [(input_index+j,j) for j in range(pop.size)]
#     input_an_proj = sim.Projection(input_ssa,pop,sim.FromListConnector(connection_list),synapse_type=sim.StaticSynapse(weight=2.),
#                                    label="input_projection_{}".format(i))

#
# n_target_pops = len(target_pops)
# n_input_pops = len(input_pops)
