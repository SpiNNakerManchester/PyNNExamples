...
import numpy as np
import pylab as plt
from signal_prep import spike_raster_plot_8,get_sub_pop_spikes,sub_pop_builder_auto
...
input_size = 5000.#len(input_spikes)#
...
for _ in range(int(input_size)):
    input_spikes.append([isi_ms*i + (np.random.rand())*(isi_ms/5.) for i in range(n_repeats)])

spike_raster_plot_8(input_spikes,plt,duration/1000.,ylim=input_size+1,title="input_activity")

n_connections = int(0.01*input_size)#76#10#
