...
def run_script(*, split: bool = False) -> None:
    ...
    if split:
        model.set_model_n_synapse_cores(1)
    ...
    ex_pop = sim.Population(
        NUM_EXCITATORY, model(**cell_params),
        label="Excitatory", additional_parameters={"seed": 2})
    in_pop = sim.Population(
        NUM_EXCITATORY / 4, model(**cell_params),
...
# combined binaries [IF_curr_exp_stdp_mad_vogels_2011_additive]
# split binaries [synapses_stdp_mad_vogels_2011_additive]
