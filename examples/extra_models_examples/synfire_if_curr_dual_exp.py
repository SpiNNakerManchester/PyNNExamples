...
def run_script(*, split: bool = False) -> None:
    ...
    if split:
        p.extra_models.IF_curr_dual_exp.set_model_n_synapse_cores(1)
    ...
    populations.append(
        p.Population(
            nNeurons, p.extra_models.IF_curr_dual_exp(**cell_params_lif),
            label='pop_1'))
    populations.append(
...
# combined binaries [IF_curr_exp_dual.aplx]
# split binaries [IF_curr_exp_dual_neuron.aplx]
