...
stim_size = 1000
target_pop_size = 20
num_pattern_neurons = 20#10#100#stim_size*0.1#100#
max_sync_pattern_neurons = num_pattern_neurons/2.
p_connect = 0.5#0.6#1.0#
p_response=probability_of_one_post_respond_to_stimulus(p_connect=p_connect,n_pattern_neurons=num_pattern_neurons,
...
number_of_firings_per_pattern = 10#1#TODO: get to root of this max limit
num_patterns = 2#1#5#
...
#calculate maximum repeat rate with at least one pattern_duration between patterns
pattern_repeat_rate = 1000./(num_patterns*pattern_duration*2)#0.5#0.05#5.#6.#
if pattern_repeat_rate>1:
    pattern_repeat_rate=int(pattern_repeat_rate)
...
for j in range(num_patterns):
    ...
    for i in range(num_pattern_presentations):
        ...
        random_variation = int(((pattern_repeat_period/num_patterns)-pattern_duration)*(np.random.rand()-0.5))#0#
        for beep in pattern:
...
if num_pattern_neurons>0:
    ...
    noise2pattern_pop = sim.Population(
       num_patterns*num_pattern_neurons, sim.SpikeSourcePoisson(rate=noise_rate, duration=duration-pattern_repeat_period),#90% duration to examine target pattern
...
target_noise = sim.Population(target_pop_size, sim.SpikeSourcePoisson(rate=noise_rate, duration=duration-pattern_repeat_period))
...
if nearest_pair:
    results_directory = '/home/rjames/Dropbox (The University of Manchester)/EarProject/Pattern_recognition' \
                        '/nearest_pair_stdp/{}_patterns_{}pre{}post_{}patternneurons_{}Hznoise_{}s_{}alpha'.format(num_patterns,stim_size,
                                                                                     target_pop_size,num_pattern_neurons,
                                                                                      int(noise_rate),int(duration/1000.),a_plus)

else:
    results_directory = '/home/rjames/Dropbox (The University of Manchester)/EarProject/Pattern_recognition' \
                        '/pair_stdp/{}_patterns_{}pre{}post_{}patternneurons_{}Hznoise_{}s_{}alpha'.format(
...
if target_pop_size <= 100:
    ...
    spike_raster_plot_8(target_data.segments[0].spiketrains,plt,duration/1000.,target_pop_size+1,0.001,
                        title="target pop activity",filepath=results_directory)
    spike_raster_plot_8(target_data.segments[0].spiketrains,plt,duration/1000.,target_pop_size+1,0.001,
                        title="target pop final activity",filepath=results_directory,xlim=(0.001*duration*0.9,0.001*duration))
else:
...
selective_neuron_search(stim_times,target_data.segments[0].spiketrains,duration,time_window=pattern_duration,
                        final_pattern_period=duration-pattern_repeat_period,plt=plt,filepath=results_directory,np=np,
